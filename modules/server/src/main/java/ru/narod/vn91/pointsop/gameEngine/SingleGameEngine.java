...
public class SingleGameEngine implements SingleGameEngineInterface {
		...
		if (ctrlSurrounding == null) { // there was no Ctrl surrounding
			...
			if (ctrlSurrounding.isEnemyCtrlFor(moveType)) {
				...
				if (!surroundedSomething) {
					...
					allSurroundings.remove(ctrlSurrounding);
					allSurroundings.add(ctrlSurrounding);

					lastMoveInfo.moveResult = MoveResult.BAD;
					lastMoveInfo.newSurroundings.add(ctrlSurrounding);
					return lastMoveInfo.moveResult;
				} else {
	...
	private boolean trySurround(Dot startDot,
		...
		boolean enemyCaptured = false;
		{
			...
			for (int x = 0; x < sizeX; x++) {
				for (int y = 0; y < sizeY; y++) {
					if ((innerTerritory[x][y]) && (field[x][y].isEnemy(
							moveType))) {
						enemyCaptured = true;
					}
				}
	...
	public boolean isRed(int x, int y) {
		return field[x][y].type == DotType.RED;
	}

	public boolean isBlue(int x, int y) {
		return field[x][y].type == DotType.BLUE;
	}
	...
	public boolean getLastDotColor() {
		return lastMoveInfo == null ||
				lastMoveInfo.moveType == MoveType.RED;
	}
	...
	public static class Surrounding extends SurroundingAbstract {
	...
	public class MoveInfo extends MoveInfoAbstract {
	...
	public class Dot extends DotAbstract {
...
//			return dotTypeIsIn(this.type, set);
		...
		int rotateClockwise(int previousDirection,
			...
			while (!this.getNeighbour(newDirection).isSurrBuilder(moveType)) {
				newDirection += 1;
			}
			return newDirection;
		}
